...
- [ ] 1. PythonTargetDetector
  ...
  - [ ] 1.1 Create `PythonTargetDetector.java` implementing `TargetDetector`
  ...
  - [ ] 1.2 Write `PythonTargetDetectorTest.java`
  ...
  - [ ] 1.3 Write `PythonDetectionProperties.java` (plain JUnit 5 parameterized)
...
- [ ] 2. PythonTarget skeleton + TargetRegistry registration + TargetId.PYTHON
  ...
  - [ ] 2.1 Add `PYTHON` to `TargetId` enum
  - [ ] 2.2 Create `PythonTarget.java` implementing `Target`
  ...
  - [ ] 2.3 Register `PythonTarget` + `PythonTargetDetector` in `TargetRegistry.defaultRegistry()`
    - Add `TargetId.PYTHON → new PythonTarget()` to the targets map
    - Add `new PythonTargetDetector()` to the detectors list
  - [ ] 2.4 Verify all existing JVM/Node tests pass without modification

- [ ] 3. Python artifact generation
  ...
  - [ ] 3.4 Create `PythonManifestGenerator.java`
    - `generateWiringManifest(BearIr, Path, String)` → `<blockKey>.wiring.json`
  - [ ] 3.5 Implement `PythonTarget.compile()` and `PythonTarget.generateWiringOnly()`
  ...
  - [ ] 3.6 Implement `PythonTarget.parseWiringManifest()` and `PythonTarget.ownedGeneratedPrefixes()`
  - [ ] 3.7 Write `PythonArtifactGeneratorTest.java`
    - Each artifact type generated correctly; generated Python parses without errors; user impl created once, not overwritten
  - [ ] 3.8 Write `ArtifactGenerationProperties.java` (plain JUnit 5 parameterized)
...
- [ ] 4. Governed roots computation
  ...
  - [ ] 4.1 Implement governed roots and file collection in `PythonTarget`
  ...
  - [ ] 4.2 Write governed roots unit tests in `PythonTargetTest.java`
    - Single block, multi-block, with `_shared`, without `_shared`, test file exclusion, extension filtering, `__init__.py` requirement
  - [ ] 4.3 Write `GovernedRootsProperties.java` (plain JUnit 5 parameterized)
...
- [ ] 5. PythonImportExtractor (AST-based)
  ...
  - [ ] 5.1 Create `PythonImportExtractor.java`
...
- [ ] 6. PythonDynamicImportDetector (AST-based)
  ...
  - [ ] 6.1 Create `PythonDynamicImportDetector.java`
  ...
  - [ ] 6.2 Write `PythonDynamicImportDetectorTest.java`
    - Detects `importlib.import_module("module")`; detects `__import__("module")`; distinguishes from static imports
  - [ ] 6.3 Write detection property in `ImportContainmentProperties.java` (plain JUnit 5 parameterized)
...
- [ ] 7. PythonImportBoundaryResolver
  ...
  - [ ] 7.1 Create `PythonImportBoundaryResolver.java`
...
- [ ] 8. PythonImportContainmentScanner
  ...
  - [ ] 8.1 Create `PythonImportContainmentScanner.java`
  ...
  - [ ] 8.2 Implement `PythonTarget.scanBoundaryBypass()` delegating to `PythonImportContainmentScanner.scan()`
  - [ ] 8.3 Write `PythonImportContainmentScannerTest.java`
  ...
  - [ ] 8.4 Write containment properties in `ImportContainmentProperties.java` (plain JUnit 5 parameterized)
...
- [ ] 9. Drift gate
  ...
  - [ ] 9.1 Implement drift checking in `PythonTarget`
  ...
  - [ ] 9.2 Write drift gate tests in `PythonTargetTest.java`
  ...
  - [ ] 9.3 Write `DriftGateProperties.java` (plain JUnit 5 parameterized)
...
- [ ] 10. impl.allowedDeps unsupported guard
  ...
  - [ ] 10.1 Implement `PythonTarget.blockDeclaresAllowedDeps()` — parse IR for `impl.allowedDeps` presence
  - [ ] 10.2 Implement guard in check pipeline: if `blockDeclaresAllowedDeps()` and target=Python → fail, exit `64`, `CODE=UNSUPPORTED_TARGET`, IR file path, remediation message
  - [ ] 10.3 Verify `pr-check` is unaffected by `impl.allowedDeps`
  - [ ] 10.4 Write guard tests in `PythonTargetTest.java`
  ...
  - [ ] 10.5 Write properties in `ImportContainmentProperties.java` or separate file (plain JUnit 5 parameterized)
...
- [ ] 11. Fixture projects + integration tests
  ...
  - [ ] 11.1 Create fixture projects under `kernel/src/test/resources/fixtures/python/`
  ...
  - [ ] 11.3 Run full JVM/Node regression test suite; confirm zero failures
