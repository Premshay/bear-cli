...
- [x] 3. Add `pr-check --all` integration tests to `MultiRootCompositionTest` (Workstream 4)
  ...
  - [x] 3.2 Implement `twoRootPrCheckDeltaLinesAreSorted`: init git repo; write `bear.blocks.yaml` with two blocks in two roots; compile both; commit; modify both IR files to add a new operation to each block; recompile; run `pr-check --all --base HEAD~1`; assert `exitCode == 5`; collect all `pr-delta:` lines from stderr (block results render to stderr when exit != 0); assert they are in deterministic sorted order within each block section
