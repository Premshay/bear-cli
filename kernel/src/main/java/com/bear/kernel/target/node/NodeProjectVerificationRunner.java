...
/**
 * Runs pnpm exec tsc --noEmit as the project verification step for Node targets.
 * Follows ReactProjectVerificationRunner as the exact structural model.
 */
public final class NodeProjectVerificationRunner {
    private static final int DEFAULT_TIMEOUT_SECONDS = 300;
    private static final String TSC_MODULE_NOT_FOUND = "Cannot find module";

    private NodeProjectVerificationRunner() {
    }

    /**
     * Runs pnpm exec tsc --noEmit in the project root.
     *
    ...
    public static ProjectTestResult run(Path projectRoot) throws IOException, InterruptedException {
        ...
        List<String> command = List.of("pnpm", "exec", "tsc", "--noEmit");
        ...
        }, "bear-node-verification-output-reader");
    ...
    /**
     * Checks if the output indicates that tsc is missing (not installed).
     * Package-private for testing.
     *
     * @param output the process output to check
     * @return true if tsc appears to be missing, false otherwise
     */
    static boolean isTscMissing(String output) {
        if (output == null) {
            return false;
        }
